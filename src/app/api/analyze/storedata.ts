...
export async function storeResumeData({
  ...
  if (sessionError) {
    console.error(
      '[storeResumeData] ❌ Error getting session:',
  ...
  console.log(
    '[storeResumeData] 🗃️ Inserting resume metadata into database...',
  );
  ...
  if (dbError) {
    console.error(
      '[storeResumeData] ❌ Database insert failed:',
      dbError.message,
    );
    return;
  }
  ...
  console.log(
    '[storeResumeData] ✅ Resume record inserted into database successfully.',
  );
}
